...
function setupCustomAudioPlayer() {
  ...
  learnerAudio.addEventListener('loadedmetadata', () => {
    ...
    if (!isFinite(learnerAudio.duration)) {
      // 크롬 MediaRecorder로 만든 오디오는 duration이 Infinity로 보고되는 경우가 있음
      // (컨테이너에 실제 길이가 기록되지 않기 때문). 끝까지 한 번 탐색(seek)해야
      // 실제 길이가 확정된다 — 이걸 안 하면 진행 바가 깨지고 타임라인 마커가
      // 전부 0% 위치로 겹쳐 보이는 문제가 간헐적으로 발생한다.
      learnerAudio.currentTime = 1e7;
      const onTimeUpdate = () => {
        learnerAudio.removeEventListener('timeupdate', onTimeUpdate);
        learnerAudio.currentTime = 0;
        finalizeDuration();
      };
      learnerAudio.addEventListener('timeupdate', onTimeUpdate);
    } else {
...
function buildTimelineMarkers() {
  ...
  if (!learnerAudio.duration || !isFinite(learnerAudio.duration)) return;
